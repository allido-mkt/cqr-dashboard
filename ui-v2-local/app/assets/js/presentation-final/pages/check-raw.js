...
import { getState, setFilters, setRawCheck } from "../state.js?v=3524";
...
function tone(status) { return ["completed", "raw_ready", "raw_updated"].includes(status) ? "ready" : ["failed", "raw_missing"].includes(status) ? "danger" : ["running", "queued"].includes(status) ? status : "warning"; }
...
function consumeHandoff() {
  ...
  try {
    ...
    setRawCheck({
      ...
      currentMonth: "", tabsFound: "-", tabsExpected: "-", missingTabs: "", rawHash: "",
...
function renderJobs(raw) {
  const jobs = raw.jobs || [];
  return jobs.length ? `<div class="table-wrap"><table><thead><tr><th>Game</th><th>Period</th><th>Status</th><th>Result</th><th>Tabs</th><th>Raw Hash</th></tr></thead><tbody>${jobs.map((job) => `<tr><td>${escapeHtml(job.game_code)}</td><td>${escapeHtml(job.period_key)}</td><td>${statusPill(tone(job.status), escapeHtml(job.status))}</td><td>${statusPill(tone(job.result_status), escapeHtml(job.result_status || "-"))}</td><td>${escapeHtml(job.tab_count_found ?? "-")} / ${escapeHtml(job.tab_count_expected ?? "-")}</td><td class="code-chip">${escapeHtml(job.raw_data_hash || "-")}</td></tr>`).join("")}</tbody></table></div>` : "";
}
...
function applyStatus(result) {
  ...
  setRawCheck({
    ...
    tabsFound: job.tab_count_found ?? "-", tabsExpected: job.tab_count_expected ?? payload.tab_count_expected ?? "-",
...
async function submit() {
  ...
  setRawCheck({
    ...
    tabsFound: "-", tabsExpected: "-", missingTabs: "", rawHash: "", finishedAt: "", errorMessage: "", jobs: [],
  });
  localStorage.removeItem(ACTIVE_KEY);
  try {
    const result = await callAuthorized("admin.n8n.raw.check", { game, month, target_game_code: game, target_month: month, target_games_csv: game, target_months_csv: month, check_mode: "manual" }, 60000);
